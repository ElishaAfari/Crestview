...
export function Sidebar() {
  ...
  const activeSuite = suiteGroups.find((group) => group.links.some((link) => isActivePath(pathname, link.href.split("?")[0], home)))?.title;
  const [openSuites, setOpenSuites] = useState<string[]>(activeSuite ? [activeSuite] : [suiteGroups[0]?.title].filter(Boolean) as string[]);
    ...
    <aside className="hidden h-full min-h-0 w-76 shrink-0 flex-col overflow-hidden bg-[var(--portal-shell)] text-white shadow-[10px_0_40px_-28px_rgba(15,23,42,0.7)] lg:flex">
      ...
      <nav className="min-h-0 flex-1 space-y-6 overflow-y-auto px-4 py-5">
        {suiteGroups.map((group) => {
          ...
          return (
            <div key={group.title}>
              <button type="button" onClick={() => setOpenSuites((current) => expanded ? current.filter((title) => title !== group.title) : [...current, group.title])} className="flex w-full items-center justify-between px-3 text-[11px] font-black uppercase tracking-normal text-cyan-100">
                <span className="flex items-center gap-2"><GroupIcon className="size-3.5" aria-hidden />{group.title}</span>
                <span aria-hidden>{expanded ? "−" : "+"}</span>
              </button>
              {expanded ? <div className="mt-2 space-y-1">
                {group.links.map((item) => {
                  const active = isActivePath(pathname, item.href.split("?")[0], home);
                  const Icon = items.find((entry) => entry.href === item.href)?.icon ?? GroupIcon;
                  return <Link key={item.href} href={item.href} className={cn("flex min-h-10 items-center gap-3 rounded-lg px-3 py-2 text-sm font-black text-white/95 transition hover:bg-white/14 hover:text-white", active && "bg-white text-[#07377f] shadow-lg shadow-slate-950/15 hover:bg-white hover:text-[#07377f]")}>
                    <span className={cn("grid size-8 shrink-0 place-items-center rounded-lg bg-white/16 text-white ring-1 ring-white/10", active && "bg-[#eef4ff] text-[#07377f] ring-[#bfd8fa]")}><Icon className="size-4" aria-hidden /></span>
