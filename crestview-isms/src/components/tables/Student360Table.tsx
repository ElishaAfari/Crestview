...
function getColumns(profileBasePath: string): ColumnDef<Student360Row>[] {
  return [
    ...
    { accessorKey: "gradeAverage", header: "Average", cell: ({ row }) => <Percent value={row.original.gradeAverage} /> },
    {
      ...
      cell: ({ row }) => (
        <div>
          <p className="font-black text-[var(--portal-text)]">{row.original.openInvoices} invoice{row.original.openInvoices === 1 ? "" : "s"}</p>
          <p className="text-xs font-bold text-[var(--portal-muted)]">{row.original.openAmount}</p>
        </div>
      )
    },
    {
      ...
      header: "Records",
      cell: ({ row }) => (
        <div className="text-sm font-black text-[var(--portal-text)]">
          {row.original.reports} reports
          <p className="text-xs font-bold text-[var(--portal-muted)]">{row.original.notes} notes | {row.original.lowGrades} alerts</p>
        </div>
      )
    },
