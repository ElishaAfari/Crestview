...
export function ReportTable({ data = [], viewBasePath }: { data?: ReportRow[]; viewBasePath?: string }) {
  const columns = useMemo<ColumnDef<ReportRow>[]>(() => [
    ...
    { accessorKey: "createdAt", header: "Created" },
    {
      id: "view",
      header: "View",
      cell: ({ row }) => viewBasePath ? (
        <Link className="portal-register-link h-9 px-3 text-xs" href={`${viewBasePath}/${row.original.id}`}>
