...
import { bulkDecideAdmissionApplicationsAction, decideAdmissionApplicationAction } from "@/features/admissions/actions";

type State = { ok: boolean; message: string };
const initialState: State = { ok: false, message: "" };

export function AdmissionDecisionControls({ applicationId }: { applicationId: string }) {
  const [state, action, pending] = useActionState(async (_: State, formData: FormData) => decideAdmissionApplicationAction(formData), initialState);
...
export function BulkAdmissionDecisionControls() {
