...
export function StudentCsvImportForm() {
  const [result, setResult] = useState<{ ok: boolean; message: string; errors?: string[]; credentials?: Array<{ name: string; studentNumber: string; temporaryPassword: string }> } | null>(null);
  ...
  return <div className="grid gap-5 lg:grid-cols-[1fr_auto]">
    <form action={submit} className="grid gap-4 rounded-xl border border-[var(--portal-border)] bg-[var(--portal-control)] p-5">
      ...
      <p className="text-xs font-semibold leading-5 text-[var(--portal-muted)]">Required information: student name and class. The importer accepts different column order, comma/semicolon/tab separators, and common headings such as <code>Learner Name</code>, <code>Admission No.</code>, <code>Grade</code>, <code>Sex</code>, and <code>Email Address</code>. External IDs such as <code>STU0826129</code> are retained for reconciliation while the platform generates its consistent <code>CIS/ST/000001</code> internal ID. Blank or repeated contact emails do not block import. Each student instead receives a unique portal account, and a secure credentials file is offered after a successful import.</p>
      <Button type="submit" disabled={pending}><Upload size={16} aria-hidden />{pending ? "Importing..." : "Import students"}</Button>
      {result ? <div className={`grid gap-2 text-sm font-bold ${result.ok ? "text-emerald-700 dark:text-emerald-300" : "text-rose-700 dark:text-rose-300"}`}><p>{result.message}</p>{result.credentials?.length ? <a className="inline-flex w-fit items-center gap-2 text-sm underline underline-offset-4" download="crestview-student-access-credentials.csv" href={`data:text/csv;charset=utf-8,${encodeURIComponent(["Student,Student ID,Temporary password", ...result.credentials.map((credential) => [credential.name, credential.studentNumber, credential.temporaryPassword].map((value) => `\"${value.replaceAll("\"", "\"\"")}\"`).join(","))].join("\n"))}`}><Download size={15} aria-hidden />Download secure student credentials</a> : null}{result.errors?.map((error) => <p key={error} className="text-xs font-semibold">{error}</p>)}</div> : null}
    </form>
