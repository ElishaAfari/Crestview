...
import { AtSign, KeyRound, Save, ShieldCheck } from "lucide-react";
import { changeOwnPasswordAction, requestOwnEmailChangeAction, updateOwnProfileAction } from "@/features/account/actions";
...
export function AccountSettingsForm({ profile }: { profile: Profile }) {
  ...
  async function saveEmail(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); setSavingEmail(true); const result = await requestOwnEmailChangeAction(new FormData(event.currentTarget)); setEmailResult(result); setSavingEmail(false); if (result.ok) event.currentTarget.reset();
  }
  ...
  return <div className="grid gap-6 xl:grid-cols-2">
    <Card><CardHeader><CardTitle>Personal profile</CardTitle><p className="text-sm font-semibold text-[var(--portal-muted)]">Update the details used across your role workspace and school communications.</p></CardHeader><CardContent><form onSubmit={saveProfile} className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="sm:col-span-2"><Label>Current sign-in email</Label><Input value={profile.email} readOnly className="bg-[var(--portal-muted-surface)]" /></div>
    ...
    <Card><CardHeader><CardTitle>Sign-in email</CardTitle><p className="text-sm font-semibold text-[var(--portal-muted)]">Parents may use this email or a ward&apos;s student ID with their own password to sign in.</p></CardHeader><CardContent><form onSubmit={saveEmail} className="grid gap-4"><div><Label>New email</Label><Input name="email" type="email" autoComplete="email" placeholder="name@example.com" required /></div><div className="flex flex-wrap items-center gap-3"><Button type="submit" variant="secondary" disabled={savingEmail}><AtSign className="size-4" aria-hidden />{savingEmail ? "Sending..." : "Change sign-in email"}</Button><ResultMessage result={emailResult} /></div></form></CardContent></Card>
