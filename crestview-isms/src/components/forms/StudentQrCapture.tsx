  ...
  async function startScan() {
    ...
    try {
      if (!window.isSecureContext) {
        setMessage(
          "Camera access requires HTTPS. Open the secure school portal URL, then try Scan QR again.",
        );
        return;
      }
      if (!navigator.mediaDevices?.getUserMedia) {
        setMessage(
          "Chrome cannot access a camera in this browser context. Check the site address and camera permissions, then try again.",
        );
        return;
      }
      ...
      if (!video) {
        stopScan();
        setMessage(
          `The camera preview could not be initialized. Type the ${entityLabel} ID instead.`,
        );
        return;
      }
      ...
      controlsRef.current = await reader.decodeFromStream(
        ...
        (result) => {
          ...
          if (!text || !activeRef.current || capturedRef.current) return;
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <div className="relative min-w-0 flex-1">
          <QrCode
            className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-blue-700 dark:text-blue-200"
            aria-hidden
          />
          <Input
            name={name}
            ...
            onBlur={(event) =>
              onValue(normalizeQrCapture(event.target.value, entity))
            }
      ...
      <video
        ...
        className={
          scanning
            ? "aspect-video w-full rounded-lg border border-[var(--portal-border)] bg-black object-cover"
            : "hidden"
        }
      />
      <div className="portal-subtle-card flex items-start gap-2 rounded-lg p-3 text-xs font-extrabold text-[var(--portal-muted)]">
        ...
        <span>
          {scanning
            ? "Camera is active. Hold the QR code inside the preview and it will capture automatically."
            : `Manual fallback accepts the ${entity === "staff" ? "staff member's CIS/STA0001 ID" : "student's CIS/ST/000001 ID"}, the QR payload, or a copied QR URL containing the ID.`}
        </span>
      </div>
      {message ? (
