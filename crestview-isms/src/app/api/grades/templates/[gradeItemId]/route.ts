...
async function canDownloadTemplate(
  userId: string,
  role: string,
  courseId: string,
...
export async function GET(
  _request: Request,
  context: { params: Promise<{ gradeItemId: string }> },
) {
  try {
    ...
    const { data: gradeItemData } = await admin
      ...
      .is("deleted_at", null)
      .maybeSingle();
    const gradeItem = gradeItemData as {
      id: string;
      title: string;
      course_id: string;
    ...
    const { data: courseData } = await admin
      .from("courses")
      .select(
        "id,classroom_id,teacher_id,academic_year_id,term,subjects(name,code),classrooms(id,name,grade_level,academic_years(name))",
      )
    ...
    if (!course)
      return NextResponse.json(
        { error: "The selected assessment is not linked to a class subject." },
        { status: 404 },
      );
    if (
      !(await canDownloadTemplate(user.id, role, course.id, course.teacher_id))
    ) {
      return NextResponse.json(
        { error: "You can only download templates for assigned classes." },
        { status: 403 },
      );
    }
    ...
    const teacher = teacherResult.data as {
      first_name: string;
      last_name: string;
    } | null;
    const academicYear =
      one(classroom?.academic_years)?.name ?? "Current Academic Year";
    ...
    ["G8", "K8", "L8", "O8"].forEach((address) => {
      sheet.getCell(address).fill = {
        type: "pattern",
        pattern: "solid",
        fgColor: { argb: address === "K8" ? "FFDC2626" : "FF174EA6" },
      };
    });
    ...
    sheet.autoFilter = {
      from: "A9",
      to: `O${firstDataRow + templateRows - 1}`,
    };
    sheet.getCell("A1").value =
      "Do not rename header columns. Fill only the yellow score/comment cells, then upload the workbook.";
    ...
    ["A", "B", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N"].forEach(
      (column) => {
    ...
    for (
      ...
      row <= firstDataRow + templateRows - 1;
      row += 1
    ) {
      ["G", "H", "I"].forEach((column) => {
        sheet.getCell(`${column}${row}`).dataValidation = {
          ...
          error:
            "Assignment, quiz, and mid-term scores must be between 0 and 10.",
        };
      });
    ...
    instructions.getRow(1).values = [
      "Crestview Grade Template",
    ...
    instructions.addRows([
      [
        "Step 1",
        "Use the Grading Report sheet. The student list and IDs are prefilled for the selected class.",
      ],
      [
        "Step 2",
        "Fill the yellow cells only: Assignment, Quiz, Mid Term, ES Mark, and Teacher Comment.",
      ],
      [
        "Step 3",
        "CA Mark, Total, Grade, and Remark calculate automatically from the workbook formulas and are recalculated again by the platform on upload.",
      ],
      [
        "Step 4",
        "Save the workbook as .xlsx and upload it back through the Subject Grade Import panel.",
      ],
      [
        "Limits",
        "Assignment <= 10, Quiz <= 10, Mid Term <= 10, End of Term Exam <= 70.",
      ],
    ]);
