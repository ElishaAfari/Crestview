...
export async function GET(
  ...
  const roleName =
    one(
      (profileData as unknown as { roles: Relation<{ name: string }> } | null)
        ?.roles,
    )?.name ?? "";
  const { data } = await admin
    .from("reports")
    .select(
      "id,term,summary,status,published_at,analysis,attendance_summary,grade_summary,attitude,punctuality,next_steps,student_id,classroom_id,students(profile_id,student_number,profiles!students_profile_id_fkey(first_name,last_name)),academic_years(name)",
    )
    .eq("id", id)
    .is("deleted_at", null)
    .maybeSingle();
  ...
  if (!(await canReadReport(auth.user.id, roleName, report)))
  ...
  function drawResultsHeader(
    columns: ReadonlyArray<{ label: string; width: number }>,
    left: number,
    rowHeight: number,
  ) {
    page.drawRectangle({
  ...
  function drawResultsTable(dataRows: typeof tableRows) {
    ...
    for (const row of dataRows) {
      const subjectLines = wrapPdfText(
        row.subject,
        bold,
        8.2,
        columns[0].width - 8,
      ).slice(0, 2);
      const remarkLines = wrapPdfText(
        row.remark,
        regular,
        7.8,
        columns[8].width - 8,
      ...
      const rowHeight = Math.max(
        34,
        Math.max(subjectLines.length, remarkLines.length) * 9 + 16,
      );
      if (y - rowHeight < 52) {
        page = pdf.addPage([595, 842]);
        y = 800;
        page.drawText("Subject Performance Continued", {
          x: left,
          y,
          size: 11,
          font: bold,
      ...
      for (const column of columns) {
        ...
        const value =
          typeof rawValue === "number" ? scoreText(rawValue) : String(rawValue);
        const font =
          column.key === "subject" ||
          column.key === "total" ||
          column.key === "gradeCode"
            ? bold
            : regular;
        ...
        const cellLines = (
          column.key === "subject"
            ...
            : column.key === "remark"
              ? remarkLines
              : wrapPdfText(value, font, size, column.width - 8)
        ).slice(0, 2);
  ...
  page.drawText("CRESTVIEW INTERNATIONAL SCHOOL", {
    x: 42,
    y,
    size: 17,
    font: bold,
  ...
  y -= 24;
  page.drawText("End of Term Academic Report", {
    x: 42,
    y,
    size: 14,
    font: bold,
    color: rgb(0.86, 0.04, 0.12),
  });
  y -= 26;
  draw(
    `Student: ${studentName}    ID: ${student?.student_number ?? text(gradeSummary.student_number, "N/A")}`,
    { strong: true },
  );
  draw(
    `Class: ${text(gradeSummary.classroom, "Unassigned")}    Academic year: ${text(one(report.academic_years)?.name, text(gradeSummary.academic_year, "Academic year"))}    Term: ${report.term}`,
    { strong: true },
  );
  draw(
    `Position: ${text(gradeSummary.position_label, text(analysis.positionLabel, "Not ranked"))}${numberValue(gradeSummary.class_size, numberValue(analysis.classSize)) ? ` of ${scoreText(numberValue(gradeSummary.class_size, numberValue(analysis.classSize)))}` : ""}    Total marks: ${scoreText(gradeSummary.total_marks)}    Average: ${scoreText(analysis.average)}%`,
    { strong: true },
  );
  y -= 8;
  ...
  draw(
    `Ranking basis: ${text(gradeSummary.ranking_basis, "Sum of total /100 marks across all recorded subjects in this class for the selected term.")}`,
    { size: 8.5 },
  );
  y -= 4;
  ...
  draw(
    `Attendance rate: ${numberText(attendance.rate)}% | Present: ${numberText(attendance.present)} | Late: ${numberText(attendance.late)} | Absent: ${numberText(attendance.absent)} | Excused: ${numberText(attendance.excused)}`,
  );
  draw(
  ...
  draw(
    `Strengths: ${
      asArray(analysis.strengths)
        .map((item) => text(item))
        .filter(Boolean)
        .join("; ") || "No strengths recorded."
    }`,
  );
  draw(
    `Areas for improvement: ${
      asArray(analysis.concerns)
        .map((item) => text(item))
        .filter(Boolean)
        .join("; ") || "No major concern recorded."
    }`,
  );
  draw(
    `Recommendations: ${
      asArray(analysis.recommendations)
        .map((item) => text(item))
        .filter(Boolean)
        .join("; ") || "Continue the current learning plan."
    }`,
  );
  draw(
    `Next steps: ${report.next_steps ?? text(analysis.nextSteps, "Review progress with the class teacher.")}`,
  );
