...
import {
  ...
  portalInviteSchema,
  resendPortalAccessSchema,
  updatePortalAccountSchema,
} from "@/lib/validations/access.schema";
...
async function resolveStudentDeliveryEmail(
...
export async function isOwnerBootstrapAvailable() {
  const admin = createAdminClient();
...
export async function bootstrapHeadAdminAction(
  _: AccessActionState,
  formData: FormData,
  ...
  if (!result.success)
    return {
      ok: false,
      message: result.error.issues[0]?.message ?? "Check the setup form.",
    };
  if (
    !matchesSecret(result.data.setupCode, process.env.CRESTVIEW_BOOTSTRAP_CODE)
  ) {
  ...
  if (count !== 0)
    return {
      ok: false,
      message:
        "Head administrator setup is already complete. Ask an administrator for an invitation.",
    };
  ...
  if (!role)
    return {
      ok: false,
      message: "The head administrator role is not configured.",
    };
  ...
  if (accountError || !account.user)
    return {
      ok: false,
      message: "The head administrator account could not be created.",
    };
  ...
  if (profileError) {
    ...
    return {
      ok: false,
      message: "The head administrator profile could not be created.",
    };
  }

  return {
    ok: true,
    message: "Head administrator account created. You can now sign in.",
  };
}

export async function bootstrapSchoolOwnerAction(
  _: AccessActionState,
  formData: FormData,
): Promise<AccessActionState> {
  const result = bootstrapAdminSchema.safeParse({
  ...
  if (!result.success)
    return {
      ok: false,
      message: result.error.issues[0]?.message ?? "Check the owner setup form.",
    };

  const setupCode =
    process.env.CRESTVIEW_OWNER_SETUP_CODE ??
    process.env.CRESTVIEW_BOOTSTRAP_CODE;
  if (!matchesSecret(result.data.setupCode, setupCode)) {
    return { ok: false, message: "The proprietor setup code is not valid." };
  }
  ...
  const { data: role } = await admin
    ...
    .single();
  if (!role)
    return { ok: false, message: "The school owner role is not configured." };
  ...
  if (count !== 0) {
    return {
      ok: false,
      message:
        "School owner setup is already complete. Ask the head administrator for access.",
    };
  }
  ...
  if (accountError || !account.user)
    return {
      ok: false,
      message: "The school owner account could not be created.",
    };
  ...
  const { error: profileError } = await admin.from("profiles").insert({
    ...
    onboarding_completed_at: new Date().toISOString(),
    metadata: {
      account_source: "owner_bootstrap",
      setup_locked_after_creation: true,
    },
  });
  ...
  if (profileError) {
    ...
    return {
      ok: false,
      message: "The school owner profile could not be created.",
    };
  }

  return {
    ok: true,
    message: "School owner account created. Proprietor setup is now closed.",
  };
}
...
export async function invitePortalUserAction(
  ...
  if (!result.success)
    return {
      ok: false,
      message: result.error.issues[0]?.message ?? "Check the invitation.",
    };
  ...
  if (
    ...
    return {
      ok: false,
      message:
        "Only the head administrator can invite owner or administrator accounts.",
    };
  }
  if (
    result.data.role === "school_owner" &&
    !(await isOwnerBootstrapAvailable())
  ) {
    return {
      ok: false,
      message:
        "School owner setup is already complete. Keep one proprietor account active from User Management.",
    };
  }
  ...
  const { data: role } = await admin
    ...
    .single();
  if (!role)
    return { ok: false, message: "The selected role is not configured." };
  ...
  const { error: auditError } = profileError
    ? { error: profileError }
  ...
  if (profileError || auditError) {
    ...
    return {
      ok: false,
      message: "The invitation record could not be created.",
    };
  }

  revalidatePath("/admin/access");
  const delivery =
    invitation.delivery === "crestview"
      ? "Crestview-branded access email"
      : "Supabase Auth access email";
  return {
    ok: true,
    message: `Portal account created. ${delivery} sent to ${invitation.deliveredTo}.`,
  };
}
...
export async function updatePortalAccountAction(
  ...
  if (!result.success)
    return {
      ok: false,
      message: result.error.issues[0]?.message ?? "Check the account settings.",
    };
  ...
  if (
    ...
    (!isActive || result.data.role !== currentRole)
  ) {
    return {
      ok: false,
      message:
        "Keep your own administrator account active with its current role.",
    };
  }
  ...
  if (
    ...
    return {
      ok: false,
      message:
        "Only the head administrator can assign or change administrator access.",
    };
  }

  const { data: selectedRole } = await admin
    .from("roles")
    .select("id")
    .eq("name", result.data.role)
    .maybeSingle();
  ...
  if (authError)
    return {
      ok: false,
      message: "The authentication account could not be updated.",
    };
  ...
  const { error: profileError } = await admin
    ...
    .eq("id", result.data.accountId);
  if (profileError)
    return { ok: false, message: "The portal profile could not be updated." };
...
export async function resendPortalAccessAction(
  _: AccessActionState,
  formData: FormData,
): Promise<AccessActionState> {
  const result = resendPortalAccessSchema.safeParse({
    accountId: String(formData.get("accountId") ?? ""),
  });
  if (!result.success)
    return {
      ok: false,
      message: result.error.issues[0]?.message ?? "Select an account.",
    };
  ...
  const { data: profileData } = await admin
    .from("profiles")
    .select(
      "id,email,first_name,last_name,role_id,is_active,metadata,roles(name)",
    )
    .eq("id", result.data.accountId)
    .maybeSingle();
  ...
  const access = await sendPortalAccessEmail({
    ...
    subject:
      roleName === "student"
        ? `${profile.first_name}'s Crestview student portal access`
        : "Your Crestview portal access link",
    accountEmailLabel:
      roleName === "student" ? "Student sign-in email" : "Sign-in email",
  });
  ...
  if (typeof invitation?.id === "string") {
    await admin
      .from("portal_invitations")
      .update({ status: "invited", expires_at: expiresAt })
      .eq("id", invitation.id);
