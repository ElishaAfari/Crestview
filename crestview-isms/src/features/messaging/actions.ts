...
export async function sendMessageAction(formData: FormData) {
  ...
  if (!result.success)
    return {
      ok: false,
      message: result.error.issues[0]?.message ?? "Check the message.",
    };
  ...
  if (!sender)
    return { ok: false, message: "Your sender profile could not be found." };
  ...
  if (!recipients.length)
    return { ok: false, message: "No active recipients were found." };

  if (role === "student" || role === "parent") {
    const allowedRecipientRoles = new Set([
      "super_admin",
      "school_owner",
      "school_admin",
      "teacher",
    ...
    const blocked = recipients.some(
      (recipient) =>
        !allowedRecipientRoles.has(one(recipient.roles)?.name ?? ""),
    );
    if (blocked)
      return {
        ok: false,
        message: "Students and parents can only message school staff.",
      };
  }

  const title =
    result.data.title ||
    `Portal message from ${sender.first_name} ${sender.last_name}`;
  ...
  if (conversationError || !conversation)
    return { ok: false, message: "The conversation could not be created." };

  const memberRows = Array.from(
    new Set([user.id, ...recipients.map((recipient) => recipient.id)]),
  ).map((profileId) => ({
  ...
  const { error: memberError } = await admin
    ...
    .insert(memberRows);
  if (memberError)
    return { ok: false, message: "Conversation members could not be saved." };
  ...
  if (messageError || !message)
    return { ok: false, message: "The message could not be saved." };
  ...
  await Promise.all([
    ...
    admin.from("email_outbox").insert(
      recipients.map((recipient) => ({
  ...
  return {
    ok: true,
    message: `Message sent to ${recipients.length} recipient${recipients.length === 1 ? "" : "s"}.`,
  };
}
