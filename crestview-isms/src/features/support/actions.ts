...
export async function createSupportTicketAction(formData: FormData) {
  ...
  if (!result.success)
    return {
      ok: false,
      message: result.error.issues[0]?.message ?? "Check the support ticket.",
    };
  ...
  if (!error && ticketData) {
    const { data: recipients } = await admin
      ...
      .is("deleted_at", null);
    const recipientIds = (
      (recipients ?? []) as unknown as Array<{ id: string }>
    )
    ...
    await admin
      ...
      .eq("event_key", "support.ticket_opened");
  }
  ...
  return error
    ? { ok: false, message: "The support ticket could not be created." }
    : { ok: true, message: `Ticket ${number} created.` };
}
...
export async function updateSupportTicketStatusAction(formData: FormData) {
  ...
  if (!result.success)
    return {
      ok: false,
      message: result.error.issues[0]?.message ?? "Check the ticket update.",
    };
  ...
  const { error } = await admin
    ...
    .update({
      ...
      priority: result.data.priority,
      assigned_to: user.id,
      resolved_at: ["resolved", "closed"].includes(result.data.status)
  ...
  revalidatePath("/it/tickets");
  return error
    ? { ok: false, message: "The ticket could not be updated." }
    : { ok: true, message: "Ticket updated." };
}
