...
export async function listStudents() {
  ...
  const { data } = await supabase
    .from("students")
    .select(
      "id,student_number,status,classrooms(name),profiles!students_profile_id_fkey(first_name,last_name)",
    )
...
export async function listAttendanceRecords() {
  ...
  const { data } = await supabase
    ...
    .select(
      "id,attendance_date,status,students(profiles!students_profile_id_fkey(first_name,last_name))",
    )
    .is("deleted_at", null)
...
export async function listAttendanceRegisters(): Promise<
...
> {
  ...
  if (role === "teacher") {
    ...
    classroomIds = Array.from(
      new Set(
        [
          ...(
            (leadCourses.data ?? []) as Array<{ classroom_id: string | null }>
          ).map((course) => course.classroom_id),
          ...(
            (assignedCourses.data ?? []) as unknown as Array<{
              courses: Relation<{ classroom_id: string | null }>;
            }>
          ).map((row) => one(row.courses)?.classroom_id),
  ...
  let query = admin
    ...
    .select(
      "id,attendance_date,status,submitted_at,counts,classrooms(name,grade_level),profiles!attendance_registers_submitted_by_fkey(first_name,last_name)",
    )
    .is("deleted_at", null)
    .order("attendance_date", { ascending: false })
    .limit(80);
    ...
    return {
      ...
      submittedAt: register.submitted_at
        ? new Intl.DateTimeFormat("en-GH", {
            dateStyle: "medium",
            timeStyle: "short",
          }).format(new Date(register.submitted_at))
        : "Not submitted",
...
export async function listAdminAttendanceMarkRoster(): Promise<
...
> {
  ...
  for (const student of (studentRows ?? []) as unknown as Array<{
    id: string;
    student_number: string;
...
export async function listGrades() {
  ...
  return records.map((record) => {
    ...
    const total = Number(
      record.total_score ?? record.percentage ?? record.score,
    );
    return {
      id: record.id,
      student: profile
      ...
      assessment:
        `${record.subject_name ?? one(course?.subjects)?.name ?? "Course"}${course ? ` - ${one(course.classrooms)?.name ?? ""}` : ""}${record.term_label ? ` (${record.term_label})` : ""}`.trim(),
      classAssessment:
        record.class_assessment_score !== null
          ? `${Number(record.class_assessment_score).toFixed(1)}/30`
          : "",
      examScore:
        record.exam_score !== null
          ? `${Number(record.exam_score).toFixed(1)}/70`
          : "",
...
export async function listParentStudents() {
  ...
  const { data } = await admin
    ...
    .select(
      "students(id,student_number,status,classrooms(name),profiles!students_profile_id_fkey(first_name,last_name))",
    )
...
export async function listParentGrades() {
  ...
  const { data } = await admin
    .from("grades")
    .select(
      "id,score,percentage,grade_code,remark,comments,assignment_score,quiz_score,midterm_score,class_assessment_score,exam_score,total_score,subject_name,term_label,grade_items(title,max_score,courses(subjects(name))),students(student_number,profiles!students_profile_id_fkey(first_name,last_name))",
    )
  ...
  return (
    (data ?? []) as unknown as Array<{
      ...
      grade_items: Relation<{
        title: string;
        max_score: number | null;
        courses: Relation<{ subjects: Relation<{ name: string }> }>;
      }>;
      students: Relation<{
        student_number: string;
        profiles: Relation<ProfileJoin>;
    ...
    return {
      ...
      student: profile
        ...
        : (student?.student_number ?? "Student"),
      assessment:
        `${record.subject_name ?? one(course?.subjects)?.name ?? "Course"}${record.term_label ? ` (${record.term_label})` : ""}`.trim(),
      classAssessment:
        record.class_assessment_score !== null
          ? `${Number(record.class_assessment_score).toFixed(1)}/30`
          : "",
...
export async function listParentReports() {
  ...
  const { data } = await admin
    .from("reports")
    .select(
      "id,term,summary,status,report_url,created_at,students(student_number,profiles!students_profile_id_fkey(first_name,last_name)),academic_years(name)",
    )
    .in("student_id", studentIds)
    .is("deleted_at", null)
    .order("created_at", { ascending: false })
...
export async function listCurrentStudentGrades() {
  ...
  const { data } = await admin
    .from("grades")
    .select(
      "id,score,percentage,grade_code,remark,comments,assignment_score,quiz_score,midterm_score,class_assessment_score,exam_score,total_score,subject_name,term_label,grade_items(title,max_score,courses(subjects(name))),students(profiles!students_profile_id_fkey(first_name,last_name))",
    )
    .eq("student_id", student.id)
    .is("deleted_at", null)
    .order("created_at", { ascending: false })
...
export async function listCurrentStudentReports() {
  ...
  return (
    ...
    return {
      ...
      student: profile
        ? `${profile.first_name} ${profile.last_name}`
        : (studentRow?.student_number ?? "Student"),
      academicYear: one(report.academic_years)?.name ?? "Academic year",
      term: report.term,
      summary: report.summary ?? "Ready for review",
      status: report.status ?? "draft",
...
export async function getStudentDashboardData() {
  ...
  const { data: studentRecord } = await admin
    ...
    .is("deleted_at", null)
    .maybeSingle();
  const student = studentRecord as {
    id: string;
    classroom_id: string | null;
  } | null;
  if (!student)
  ...
  const [grades, attendance, courses, invoices, reports] = await Promise.all([
    admin
      .from("grades")
      .select("score,percentage,total_score,grade_items(max_score)")
      .eq("student_id", student.id)
      .is("deleted_at", null),
    admin
      .from("attendance_records")
  ...
  const gradeRows = (grades.data ?? []) as unknown as Array<{
    score: number;
    percentage: number | null;
    total_score: number | null;
    grade_items: Relation<{ max_score: number }>;
  }>;
          ...
          (sum, row) =>
            ...
            Number(
              row.total_score ??
                row.percentage ??
                (Number(row.score) /
                  Number(one(row.grade_items)?.max_score ?? 100)) *
                  100,
            ),
          0,
        ) / gradeRows.length,
  ...
  const attendanceRate = attendanceRows.length
    ...
    : 0;
  const courseIds = ((courses.data ?? []) as Array<{ id: string }>).map(
    (course) => course.id,
  );
...
export async function getTeacherDashboardData() {
  ...
  const [leadCourses, assignedCourses] = await Promise.all([
    ...
    admin
      .from("teacher_assignments")
      .select("courses(id,classroom_id)")
      .eq("teacher_id", user.id)
      .is("deleted_at", null),
  ]);
  const courseMap = new Map<string, { id: string; classroom_id: string }>();
  for (const course of (leadCourses.data ?? []) as Array<{
    id: string;
    classroom_id: string;
  }>)
    courseMap.set(course.id, course);
  for (const row of (assignedCourses.data ?? []) as unknown as Array<{
    courses: Relation<{ id: string; classroom_id: string }>;
  }>) {
  ...
  const classroomIds = Array.from(
    new Set(courseRows.map((course) => course.classroom_id).filter(Boolean)),
  );
  const [assignments, grades, attendance] = await Promise.all([
    ...
    courseIds.length
      ? admin
          .from("grades")
          .select("grade_items!inner(course_id)", {
            count: "exact",
            head: true,
          })
          .in("grade_items.course_id", courseIds)
          .is("deleted_at", null)
      : Promise.resolve({ count: 0 }),
    classroomIds.length
      ? admin
...
export async function listFamilyInvoices() {
  ...
  const { data } = await admin
    .from("invoices")
    .select(
      "id,invoice_number,title,amount,currency,status,due_date,students(student_number,profiles!students_profile_id_fkey(first_name,last_name))",
    )
    .in("student_id", studentIds)
    .is("deleted_at", null)
...
export async function listFamilyDailyFeePayments(): Promise<
  DailyFeePaymentRow[]
> {
  const { user } = await requireRoles(["parent"]);
  const admin = createAdminClient();
  ...
  const { data } = await admin
    ...
    .select(
      "id,payment_date,student_number,amount,currency,method,status,reference,notes,students(profiles!students_profile_id_fkey(first_name,last_name),classrooms(name,grade_level))",
    )
    .in("student_id", studentIds)
    .is("deleted_at", null)
...
export async function listCurrentStudentDailyFeePayments(): Promise<
  DailyFeePaymentRow[]
> {
  const { user } = await requireRoles(["student"]);
  const admin = createAdminClient();
  const { data: studentRecord } = await admin
    .from("students")
    .select(
      "id,student_number,classrooms(name,grade_level),profiles!students_profile_id_fkey(first_name,last_name)",
    )
    .eq("profile_id", user.id)
    .is("deleted_at", null)
    .maybeSingle();
  ...
  const { data } = await admin
    .from("daily_fee_payments")
    .select(
      "id,payment_date,student_number,amount,currency,method,status,reference,notes",
    )
    .eq("student_id", student.id)
    .is("deleted_at", null)
    .order("payment_date", { ascending: false })
...
export async function listAssignmentsForCurrentRole() {
  ...
  const { data: roleRecord } = await admin
    ...
    .maybeSingle();
  const roleName = one(
    (roleRecord as unknown as { roles: Relation<{ name: string }> } | null)
      ?.roles,
  )?.name;
  ...
  if (roleName === "teacher") {
    const { data } = await admin
      ...
      .eq("teacher_id", user.id)
      .is("deleted_at", null);
    courseIds = ((data ?? []) as Array<{ id: string }>).map(
      (course) => course.id,
    );
  } else {
    ...
    if (roleName === "student") {
      ...
      classroomIds = (
        (links ?? []) as unknown as Array<{
          students: Relation<{ classroom_id: string | null }>;
        }>
      )
  ...
  const { data } = await admin
    .from("assignments")
    .select(
      "id,title,due_at,max_score,courses(subjects(name),classrooms(name))",
    )
    .in("course_id", courseIds)
    .is("deleted_at", null)
...
export async function listTeacherFormOptions(): Promise<{
  courses: SelectOption[];
  gradeItems: SelectOption[];
  students: SelectOption[];
  classrooms: SelectOption[];
  gradeImportContexts: GradeImportContext[];
}> {
  const { user } = await requireRoles(["teacher"]);
  const admin = createAdminClient();
  const [leadCourses, assignedCourses] = await Promise.all([
    admin
      .from("courses")
      .select(
        "id,term,classroom_id,subjects(name),classrooms(name,grade_level)",
      )
      .eq("teacher_id", user.id)
      .is("deleted_at", null)
      .order("term"),
    admin
      .from("teacher_assignments")
      .select(
        "courses(id,term,classroom_id,subjects(name),classrooms(name,grade_level))",
      )
      .eq("teacher_id", user.id)
      .is("deleted_at", null),
  ]);
  ...
  for (const course of (leadCourses.data ?? []) as unknown as CourseOptionRow[])
    courseMap.set(course.id, course);
  for (const row of (assignedCourses.data ?? []) as unknown as Array<{
    courses: Relation<CourseOptionRow>;
  }>) {
    const course = one(row.courses);
    if (course) courseMap.set(course.id, course);
  }
  ...
  const [gradeItems, students] = await Promise.all([
    courseIds.length
      ? admin
          .from("grade_items")
          .select(
            "id,title,category,max_score,course_id,courses(id,classroom_id,term,subjects(name),classrooms(id,name,grade_level))",
          )
          .in("course_id", courseIds)
          .is("deleted_at", null)
          .order("created_at", { ascending: false })
    ...
    classroomIds.length
      ? admin
          .from("students")
          .select(
            "id,student_number,profiles!students_profile_id_fkey(first_name,last_name)",
          )
          .in("classroom_id", classroomIds)
          .is("deleted_at", null)
          .order("student_number")
  ...
  for (const student of (importStudents ?? []) as unknown as Array<{
    id: string;
    student_number: string;
    classroom_id: string;
    profiles: Relation<ProfileJoin>;
  ...
  const gradeItemRows = (gradeItems.data ?? []) as unknown as Array<{
    ...
    courses: Relation<{
      ...
      term: string;
      subjects: Relation<{ name: string }>;
      classrooms: Relation<{ id: string; name: string; grade_level: string }>;
    }>;
  }>;
  const importRows = gradeItemRows.filter(
    (item) =>
      item.category === "term_report" || Number(item.max_score ?? 100) === 100,
  );
  ...
  return {
    ...
    gradeItems: gradeItemRows.map((item) => {
      ...
      return {
        id: item.id,
        label:
          `${item.title} - ${one(course?.subjects)?.name ?? "Course"} ${one(course?.classrooms)?.name ?? ""}`.trim(),
      };
    }),
    students: (
      (students.data ?? []) as unknown as Array<{
        id: string;
        student_number: string;
        profiles: Relation<ProfileJoin>;
      }>
      ...
      return {
        id: student.id,
        label: `${profile ? `${profile.first_name} ${profile.last_name}` : "Student"} (${student.student_number})`,
      };
    }),
...
export async function listTeacherAttendanceRoster(): Promise<
  TeacherAttendanceCourse[]
> {
  const { user } = await requireRoles(["teacher"]);
  const admin = createAdminClient();
  ...
  for (const course of (leadCourses.data ?? []) as unknown as CourseRow[])
    courseMap.set(course.id, course);
  for (const row of (assignedCourses.data ?? []) as unknown as Array<{
    courses: Relation<CourseRow>;
  }>) {
    const course = one(row.courses);
    if (course) courseMap.set(course.id, course);
  }
  ...
  const courses = Array.from(courseMap.values()).sort((a, b) => {
    ...
    const classCompare =
      `${classroomA?.grade_level ?? ""} ${classroomA?.name ?? ""}`.localeCompare(
        `${classroomB?.grade_level ?? ""} ${classroomB?.name ?? ""}`,
        undefined,
        { numeric: true },
      );
    if (classCompare) return classCompare;
    const subjectCompare = (one(a.subjects)?.name ?? "").localeCompare(
      one(b.subjects)?.name ?? "",
    );
  ...
  const classroomIds = Array.from(
    new Set(courses.map((course) => course.classroom_id).filter(Boolean)),
  );
...
export async function listTeacherClassRosters(): Promise<TeacherClassRoster[]> {
  ...
  const [leadCourses, assignedCourses] =
    role === "teacher"
      ? await Promise.all([
          admin
            .from("courses")
            .select(
              "id,classroom_id,academic_year_id,teacher_id,subjects(name),classrooms(id,name,grade_level,academic_year_id,academic_years(name))",
            )
            .eq("teacher_id", user.id)
            .is("deleted_at", null),
          admin
            .from("teacher_assignments")
            .select(
              "courses(id,classroom_id,academic_year_id,teacher_id,subjects(name),classrooms(id,name,grade_level,academic_year_id,academic_years(name)))",
            )
            .eq("teacher_id", user.id)
            .is("deleted_at", null),
        ])
  ...
  for (const course of assignedCoursesList) {
    ...
    classKeys.set(key, {
      classroomId: classroom.id,
      academicYearId:
        course.academic_year_id ?? classroom.academic_year_id ?? null,
  ...
  const classroomIds = Array.from(
    new Set(classrooms.map((item) => item.classroomId)),
  );
  ...
  if (academicYearIds.length)
    subjectQuery = subjectQuery.in("academic_year_id", academicYearIds);
  ...
  for (const row of (subjectRows.data ?? []) as unknown as Array<{
    classroom_id: string;
    academic_year_id: string | null;
    subjects: Relation<{ name: string }>;
  }>) {
  ...
  for (const student of (studentRows.data ?? []) as unknown as Array<{
    id: string;
    student_number: string;
    classroom_id: string;
    profiles: Relation<ProfileJoin>;
  ...
  return classrooms
    ...
    .map((classroom) => {
      ...
      return {
        ...classroom,
        subjects: Array.from(
          subjectsByClass.get(key) ?? new Set<string>(),
        ).sort(),
