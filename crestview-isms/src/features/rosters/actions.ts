...
async function createStudentFromRoster(
  ...
  if (!isSupportedStudentNumber(studentNumber)) {
    throw new Error(
      `${input.firstName.trim()} ${input.lastName.trim()} needs a CIS/ST/000001-style student ID or a blank ID for automatic generation.`,
    );
  }
    ...
    if (wasPending) {
      ...
      if (profile?.email) {
        ...
        const access = await sendPortalAccessEmail({
          ...
          deliveryEmail: deliveryEmail ?? profile.email,
