...
const jobPostingSchema = z.object({
  ...
  description: z
    .string()
    .trim()
    .min(20, "Describe the role and expected fit.")
    .max(4000),
...
export async function createJobPostingAction(formData: FormData) {
  ...
  if (!result.success)
    return {
      ok: false,
      message: result.error.issues[0]?.message ?? "Check the job posting.",
    };
  ...
  if (error || !posting)
    return { ok: false, message: "The job posting could not be created." };
  ...
  await Promise.all([
    ...
    createWorkflowTask({
      ...
      description:
        "Monitor website recruitment submissions and shortlist candidates for this open posting.",
      priority: "normal",
      createdBy: user.id,
      relatedTable: "job_postings",
      relatedRecordId: posting.id,
      metadata: {
        source: "job_posting_create",
        public_posting: result.data.isActive,
      } satisfies Json,
    }),
  ]);
  ...
  return {
    ok: true,
    message: "Job posting created and published to the recruitment page.",
  };
}
...
function staffRoleForTitle(title: string | null | undefined): RoleName {
  ...
  if (value.includes("library") || value.includes("librarian"))
    return "librarian";
  if (
    value.includes("it ") ||
...
function requestedStaffRole(
  ...
  return allowed.has(value as RoleName)
    ? (value as RoleName)
    : staffRoleForTitle(fallbackTitle);
}
...
export async function submitJobApplicationAction(formData: FormData) {
  ...
  if (error || !applicationRecord)
    return {
      ok: false,
      message:
        "We could not submit your application. Please call the school for assistance.",
    };
  ...
  return {
    ok: true,
    message: "Application received. The school team will review it shortly.",
  };
}
...
async function assignTeacherToClass(
  ...
  const leadlessCourseIds = courseRows
    .filter((course) => !course.teacher_id)
    .map((course) => course.id);
  if (leadlessCourseIds.length)
    await admin
      .from("courses")
      .update({ teacher_id: profileId })
      .in("id", leadlessCourseIds);
}

async function createStaffFromApplication(
  ...
  const { data: application } = await admin
    .from("job_applications")
    .select(
      "id,first_name,last_name,email,phone,status,job_postings(title,employment_type)",
    )
    .eq("id", applicationId)
    .maybeSingle();
  const record = application as unknown as {
    ...
    status: string;
    job_postings:
      | { title: string | null; employment_type: string | null }
      | { title: string | null; employment_type: string | null }[]
      | null;
  } | null;
  if (!record)
  ...
  if (record.status === "hired")
    return { ok: true, message: "This applicant has already been accepted." };
  ...
  if (existingProfile)
    return {
      ok: false,
      message:
        "A portal profile already exists for this email. Use User Management to adjust access.",
    };
  ...
  const { data: role } = await admin
    ...
    .maybeSingle();
  if (!role)
    return { ok: false, message: "The matching staff role is not configured." };
  ...
  const invite = await createPortalInvitation({
    ...
    metadata: {
      application_source: "recruitment",
      account_source: "recruitment_acceptance",
  ...
  const { error: invitationError } =
    profileError || staffProfileError
      ? { error: profileError ?? staffProfileError }
  ...
  if (roleName === "teacher") {
    await assignTeacherToClass(
      invite.user.id,
      classroomId && z.string().uuid().safeParse(classroomId).success
        ? classroomId
        : null,
      actorId,
    );
  ...
  return {
    ok: true,
    message: `${record.first_name} ${record.last_name} accepted and invited as ${roleName.replaceAll("_", " ")}. ${delivery} sent to ${invite.deliveredTo}.`,
  };
}
...
export async function decideJobApplicationAction(formData: FormData) {
  ...
  if (decision === "accept") {
    const result = await createStaffFromApplication(
      applicationId,
      user.id,
      role,
      classroomId,
