...
import {
  findOperationsWorkspace,
  type OperationsCreateField,
...
function addComputedValues(
  ...
  switch (table) {
    ...
    case "communication_campaigns":
      ...
      {
        const audienceRoles =
          Array.isArray(record.audience_roles) && record.audience_roles.length
    ...
    case "student_service_payments":
      record.payment_number = reference(
        record.service_type === "extra_classes" ? "EXT" : "FEED",
      );
      record.recorded_by = userId;
      if (!record.currency) record.currency = "GHS";
      break;
    case "student_service_adjustments":
      record.adjustment_number = reference(
        record.service_type === "extra_classes" ? "EXT-ADJ" : "FEED-ADJ",
      );
      break;
  ...
  record.metadata = {
    ...(record.metadata as object | undefined),
    source: "operations_register",
  } satisfies Json;
}

export async function createOperationsRecordAction(
  workspaceKey: string,
  moduleKey: string,
  ...
  if (!workspace || !workspaceModule)
    return {
      ok: false,
      message: "This operations register is not configured.",
    };
  if (!workspaceModule.createFields?.length)
    return {
      ok: false,
      message:
        "This register is read-only because it needs a linked record from another suite.",
    };

  const requiredError = validateRequired(
    workspaceModule.createFields,
    formData,
  );
  ...
  if (
    workspaceModule.filter &&
    record[workspaceModule.filter.key] === undefined
  ) {
  ...
  if (error)
    return {
      ok: false,
      message: `Could not create ${workspaceModule.label.toLowerCase()} record. Check required fields and try again.`,
    };
