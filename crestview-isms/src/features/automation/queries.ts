...
async function teacherClassroomIds(
  admin: ClassroomScopedClient,
  teacherId: string,
) {
...
function mapStudent360Record(row: {
  ...
  return {
    ...
    classroom: row.classroom_name
      ? `${row.grade_level ?? "Class"} - ${row.classroom_name}`
      : "Unassigned",
    status: row.status,
...
export async function listWorkflowTasksForCurrentRole(
  limit = 80,
  ...
  const privileged = isAdminRole(role);
  ...
  let query = admin
    ...
    .select(
      "id,task_number,title,workflow_key,status,priority,due_at,assigned_to,created_by,assignee_name,student_name,student_number,classroom_name,grade_level,related_table,related_record_id",
    )
  ...
  return (
    ...
    student: task.student_name
      ? `${task.student_name}${task.student_number ? ` (${task.student_number})` : ""}`
      : "Not linked",
    classroom: task.classroom_name
      ? `${task.grade_level ?? "Class"} - ${task.classroom_name}`
      : "Not linked",
    related: task.related_table
      ? `${task.related_table}${task.related_record_id ? `/${task.related_record_id.slice(0, 8)}` : ""}`
      : "Manual",
  }));
}
...
export async function listAutomationRules() {
  ...
  const { data } = await admin
    ...
    .select(
      "id,name,event_key,scope,description,is_active,priority,last_triggered_at",
    )
    .is("deleted_at", null)
...
export async function listStudent360Overview(): Promise<Student360Row[]> {
  ...
  let query = admin
    ...
    .select(
      "student_id,student_number,first_name,last_name,classroom_name,grade_level,status,attendance_rate,attendance_rate_30_days,grade_average,low_grade_count,open_invoice_count,open_invoice_amount,report_count,note_count,risk_level",
    )
...
export async function getStudent360Detail(
  studentId: string,
): Promise<Student360Detail | null> {
  const { user, role } = await requireRoles([
    "super_admin",
    "school_admin",
    "teacher",
  ...
  if (role === "teacher") {
    ...
    if (!overview.classroom_id || !classroomIds.includes(overview.classroom_id))
      return null;
  }
  ...
  const [
    ...
    admin
      ...
      .select(
        "relationship,profiles!parent_students_parent_profile_id_fkey(id,first_name,last_name,email,phone)",
      )
      .eq("student_id", studentId)
      .is("deleted_at", null),
    admin
      .from("grades")
      .select(
        "id,score,percentage,total_score,grade_code,remark,comments,assignment_score,quiz_score,midterm_score,class_assessment_score,exam_score,subject_name,term_label,grade_items(title,courses(subjects(name),classrooms(name)))",
      )
      .eq("student_id", studentId)
      .is("deleted_at", null)
      .order("created_at", { ascending: false })
    ...
    admin
      ...
      .select(
        "id,attendance_date,status,notes,profiles!attendance_records_recorded_by_fkey(first_name,last_name)",
      )
      .eq("student_id", studentId)
      .is("deleted_at", null)
    ...
    admin
      .from("reports")
      .select(
        "id,term,summary,status,report_url,created_at,academic_years(name)",
      )
      .eq("student_id", studentId)
      .is("deleted_at", null)
      .order("created_at", { ascending: false })
    ...
    admin
      ...
      .select(
        "id,note_type,title,body,visibility,created_at,profiles!student_360_notes_created_by_fkey(first_name,last_name)",
      )
      .eq("student_id", studentId)
      .is("deleted_at", null)
      .order("created_at", { ascending: false })
    ...
    admin
      .from("workflow_task_overview")
      .select(
        "id,task_number,title,workflow_key,status,priority,due_at,assignee_name,student_name,student_number,classroom_name,grade_level,related_table,related_record_id",
      )
      .eq("student_id", studentId)
      .order("created_at", { ascending: false })
      .limit(30),
  ...
  const guardians = (
    (guardianResult.data ?? []) as unknown as Array<{
      relationship: string;
  ...
  const grades = (
    (gradeResult.data ?? []) as unknown as Array<{
      id: string;
      score: number;
    ...
    const total = Number(
      grade.total_score ?? grade.percentage ?? grade.score ?? 0,
    );
    return {
      id: grade.id,
      subject:
        grade.subject_name ??
        one(course?.subjects)?.name ??
        gradeItem?.title ??
        "Subject",
      term: grade.term_label ?? "Term",
      classAssessment:
        grade.class_assessment_score !== null
          ? `${Number(grade.class_assessment_score).toFixed(1)}/30`
          : "-",
      examScore:
        grade.exam_score !== null
          ? `${Number(grade.exam_score).toFixed(1)}/70`
          : "-",
      total,
  ...
  const attendance = (
    (attendanceResult.data ?? []) as unknown as Array<{
      id: string;
  ...
  const reports = (
    (reportResult.data ?? []) as unknown as Array<{
      id: string;
      term: string;
      summary: string | null;
      status: string | null;
  ...
  const notes = (
    (noteResult.data ?? []) as unknown as Array<{
      id: string;
      note_type: string;
      title: string;
      body: string;
...
export async function listTaskFormOptions() {
  ...
  return {
    assignees: (
      ...
      .map((profile) => ({
        id: profile.id,
        label: `${profile.first_name} ${profile.last_name} (${(one(profile.roles)?.name ?? "staff").replaceAll("_", " ")})`,
      })),
    students: (
