...
export async function submitAdmissionAction(formData: FormData) {
  const values = {
    ...
    submittedBirthCertificate: formString(
      formData,
      "submittedBirthCertificate",
    ),
    ...
    consentEmergencyTreatment: formString(
      formData,
  ...
  if (!result.success) {
    return {
      ok: false,
      message: result.error.issues[0]?.message ?? "Invalid application.",
    };
  }
  ...
  const guardianRows = [
    result.data.fatherName
      ? {
          ...
          is_primary:
            result.data.guardianRelationship.toLowerCase() === "father",
        }
      : null,
    result.data.motherName
      ? {
          ...
          is_primary:
            result.data.guardianRelationship.toLowerCase() === "mother",
        }
      : null,
    {
      ...
      relationship:
        result.data.guardianRelationship === "Other" &&
        result.data.guardianRelationshipOther
          ? result.data.guardianRelationshipOther
          : result.data.guardianRelationship,
  ...
  if (error || !applicationRecord)
    return {
      ok: false,
      message:
        "We could not submit the application. Please call the school for assistance.",
    };
  const guardiansWithApplication = guardianRows.map((row) => ({
    ...row,
  ...
  const { error: guardianError } = await admin
    ...
    .insert(guardiansWithApplication);
  const { error: documentError } = documents.length
    ? await admin.from("admission_documents").insert(
  ...
  if (guardianError || documentError) {
    ...
    return {
      ok: false,
      message:
        "We could not save the full application. Please call the school for assistance.",
    };
  }
  ...
  await notifyAdministrators(
    ...
    {
      admission_application_id: applicationRecord.id,
      source: "school_website",
    },
  );
...
async function ensureParentAccount(
  ...
  if (!role)
    return {
      ok: false as const,
      message: "The parent role is not configured.",
    };
  const { data: primaryGuardian } = await admin
    ...
    .eq("is_primary", true)
    .maybeSingle();
  const guardian = primaryGuardian as {
    first_name: string;
    last_name: string;
    phone: string | null;
  } | null;
  ...
  const { error: invitationError } = profileError
    ? { error: profileError }
  ...
  if (profileError || invitationError) {
    ...
    return {
      ok: false as const,
      message: "The guardian portal invitation could not be recorded.",
    };
  }
  await admin.from("account_lifecycle_records").insert({
    ...
    snapshot: {
      admission_application_id: application.id,
      access_method: "secure_invite_link",
      delivery: account.delivery,
      delivered_to: account.deliveredTo,
    },
  });
  return {
    ok: true as const,
    ...
    delivery: account.delivery,
    deliveredTo: account.deliveredTo,
  };
}
...
async function acceptAdmission(applicationId: string, actorId: string) {
  ...
  if (!application)
    return { ok: false, message: "The application no longer exists." };
  if (application.status === "accepted")
    return { ok: true, message: "This application has already been accepted." };
  ...
  const { data: classroom } = await admin
    ...
    .select("id")
    .or(
      `grade_level.ilike.%${application.applying_grade}%,name.ilike.%${application.applying_grade}%`,
    )
    .is("deleted_at", null)
    .limit(1)
    .maybeSingle();
  ...
  if (accountError || !account.user)
    return {
      ok: false,
      message: "The student portal record could not be prepared.",
    };
  ...
  const { error: profileError } = await admin.from("profiles").insert({
    ...
    emergency_contact:
      ...
      !Array.isArray(application.metadata)
        ? (application.metadata.emergency_contact as Json)
        : null,
    is_active: false,
    metadata: {
      account_source: "admission_acceptance",
  ...
  const { data: studentData, error: studentError } = profileError
    ...
    : await admin
        ...
        .insert({
          ...
          metadata: {
            applying_grade: application.applying_grade,
            guardian_email: application.guardian_email,
          },
        })
        .select("id")
  ...
  await createWorkflowTask({
    ...
    description:
      "Verify parent access, confirm class placement, check required documents, and prepare the first invoice.",
    priority: "high",
  ...
  await admin
    ...
    .eq("event_key", "admission.accepted");
  await admin.from("admission_status_history").insert({
  ...
  await admin.from("account_lifecycle_records").insert({
    ...
    action: "created",
    reason:
      "Student record created from accepted admission; portal access pending class teacher activation",
    performed_by: actorId,
    snapshot: {
      admission_application_id: application.id,
      student_number: studentNumber,
    ...
    if (!error) {
      await admin
        ...
        .insert({
          ...
          to_status: "rejected",
          changed_by: user.id,
          reason: "Denied from administrator review",
        });
    }
    revalidatePath("/admin/admissions");
    return error
      ? { ok: false, message: "The application could not be denied." }
      : { ok: true, message: "Application denied." };
  }
  return { ok: false, message: "Choose accept or deny." };
}

export async function bulkDecideAdmissionApplicationsAction(
  formData: FormData,
) {
  const decision = String(formData.get("decision") ?? "");
  const { user } = await requireRoles(["super_admin", "school_admin"]);
  ...
  if (!ids.length)
    return { ok: false, message: "No submitted applications are waiting." };
  ...
  if (decision === "deny") {
    ...
    revalidatePath("/admin/admissions");
    return error
      ? { ok: false, message: "Applications could not be denied." }
      : { ok: true, message: `${ids.length} applications denied.` };
  }
  ...
  if (decision === "accept") {
    ...
    return {
      ok: failed === 0,
      message: `${accepted} accepted${failed ? `, ${failed} need manual review` : ""}.`,
    };
  }
