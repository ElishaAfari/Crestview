...
const requestedAccountTotal = Number.parseInt(
  env.BETA_ACCOUNT_TOTAL || "200",
  10,
);

if (!supabaseUrl || !serviceRoleKey) {
  throw new Error(
    "Missing NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY/SUPABASE_SECRET_KEY for beta testing.",
  );
}
...
async function selectColumn(table, column, filter) {
  ...
  const { data, error } = await request;
  if (error)
    throw new Error(`${table} cleanup select failed: ${error.message}`);
  return (data ?? []).map((row) => row[column]).filter(Boolean);
}

async function selectColumnByIn(table, column, filterColumn, values) {
  const uniqueValues = [...new Set(values.filter(Boolean))];
  const output = [];
  for (let index = 0; index < uniqueValues.length; index += 100) {
    const chunk = uniqueValues.slice(index, index + 100);
    output.push(
      ...(await selectColumn(table, column, (query) =>
...
async function deleteWhere(table, filter) {
  ...
  if (error)
    throw new Error(`${table} cleanup delete failed: ${error.message}`);
}
...
async function cleanupRun(cleanupRunId) {
  ...
  const academicYearIds = await selectColumn("academic_years", "id", (query) =>
    query.eq("name", `Crestview Beta ${cleanupRunId}`),
  );
  const profileIds = await selectColumn("profiles", "id", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  const studentIds = await selectColumn("students", "id", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  const staffProfileIds = await selectColumn("staff_profiles", "id", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  const courseIds = academicYearIds.length
    ? await selectColumn("courses", "id", (query) =>
        query.in("academic_year_id", academicYearIds),
      )
    : [];
  const classroomIds = academicYearIds.length
    ? await selectColumn("classrooms", "id", (query) =>
        query.in("academic_year_id", academicYearIds),
      )
    : [];
  const invoiceIds = await selectColumn("invoices", "id", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  const gradeItemIds = await selectColumn("grade_items", "id", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  const registerIds = await selectColumn(
    "attendance_registers",
    "id",
    (query) => query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  const jobApplicationIds = await selectColumn(
    "job_applications",
    "id",
    (query) => query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  const ticketIds = await selectColumn("support_tickets", "id", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  const campaignIds = await selectColumn(
    "communication_campaigns",
    "id",
    (query) => query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  const bookIds = await selectColumn("library_books", "id", (query) =>
    query.ilike("isbn", `BETA-${cleanupRunId}-%`),
  );
  const copyIds = bookIds.length
    ? await selectColumn("library_copies", "id", (query) =>
        query.in("book_id", bookIds),
      )
    : [];
  const payrollPeriodIds = await selectColumn(
    "payroll_periods",
    "id",
    (query) => query.ilike("name", `%${cleanupRunId}%`),
  );
  const conversationIdsByTitle = await selectColumn(
    "conversations",
    "id",
    (query) => query.ilike("title", `%${cleanupRunId}%`),
  );
  const conversationIdsByMember = profileIds.length
    ? await selectColumnByIn(
  ...
  await deleteWhere("communication_recipients", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteWhere("communication_campaigns", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteWhere("workflow_tasks", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteWhere("student_360_notes", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteWhere("notifications", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteWhere("account_lifecycle_records", (query) =>
    query.contains("snapshot", { beta_run_id: cleanupRunId }),
  );
  await deleteByIn("ai_analytics", "student_id", studentIds);
  await deleteWhere("reports", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteWhere("grade_import_batches", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteByIn("grades", "grade_item_id", gradeItemIds);
  await deleteWhere("grade_items", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteByIn("attendance_records", "register_id", registerIds);
  await deleteWhere("attendance_registers", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteByIn("daily_fee_payments", "student_id", studentIds);
  await deleteWhere("daily_fee_plans", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteByIn("invoice_items", "invoice_id", invoiceIds);
  await deleteByIn("payments", "invoice_id", invoiceIds);
  await deleteWhere("invoices", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteWhere("billing_batches", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteByIn(
  ...
  await deleteWhere("job_applications", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteWhere("job_postings", (query) =>
    query.ilike("title", `%Beta ${cleanupRunId}%`),
  );
  await deleteWhere("admission_applications", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  await deleteWhere("events", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  ...
  await deleteWhere("class_roster_snapshots", (query) =>
    query.in("academic_year_id", academicYearIds),
  );
  await deleteWhere("staff_class_assignments", (query) =>
    query.contains("metadata", { beta_run_id: cleanupRunId }),
  );
  ...
  await deleteWhere("school_days", (query) =>
    query.in("academic_year_id", academicYearIds),
  );
  await deleteWhere("terms", (query) =>
    query.in("academic_year_id", academicYearIds),
  );
  await deleteByIn("staff_profiles", "id", staffProfileIds);
  await deleteByIn("audit_logs", "actor_id", profileIds);
  await deleteByIn("audit_logs", "record_id", [
    ...profileIds,
    ...studentIds,
    ...staffProfileIds,
  ]);
  ...
  for (const profileId of profileIds) {
    ...
    if (error && !/User not found/i.test(error.message))
      throw new Error(`auth cleanup failed for ${profileId}: ${error.message}`);
  }

  await deleteByIn("academic_years", "id", academicYearIds);
  process.stdout.write(
    `[beta:${cleanupRunId}] cleanup removed ${profileIds.length} profiles and ${studentIds.length} students\n`,
  );
}
...
function gradeFor(total) {
  if (total >= 75)
    return { code: "A1", remark: "Excellent", points: 1, passing: true };
  if (total >= 70)
    return { code: "B2", remark: "Very good", points: 2, passing: true };
  if (total >= 65)
    return { code: "B3", remark: "Good", points: 3, passing: true };
  if (total >= 60)
    return { code: "C4", remark: "Credit", points: 4, passing: true };
  if (total >= 55)
    return { code: "C5", remark: "Credit", points: 5, passing: true };
  if (total >= 50)
    return { code: "C6", remark: "Credit", points: 6, passing: true };
  if (total >= 45)
    return { code: "D7", remark: "Pass", points: 7, passing: true };
  if (total >= 40)
    return { code: "E8", remark: "Pass", points: 8, passing: true };
  return {
    code: "F9",
    remark: "Needs intensive support",
    points: 9,
    passing: false,
  };
}
...
const rolesToCreate = [
  ...
  [
    ...
    "Executive client proprietor access",
  ],
...
function makeRolePlan(accountTotal) {
  const safeTotal =
    Number.isFinite(accountTotal) && accountTotal >= 50 ? accountTotal : 200;
  const fixed = { super_admin: 1, school_owner: 1 };
  const flexibleRoles = Object.keys(baseRolePlan).filter(
    (role) => !(role in fixed),
  );
  const flexibleBaseTotal = flexibleRoles.reduce(
    (sum, role) => sum + baseRolePlan[role],
    0,
  );
  const flexibleTarget =
    safeTotal - Object.values(fixed).reduce((sum, count) => sum + count, 0);
  const scaled = {};
  let assigned = Object.values(fixed).reduce((sum, count) => sum + count, 0);
  ...
  const remainders = flexibleRoles
    .map((role) => ({
      role,
      remainder:
        (baseRolePlan[role] / flexibleBaseTotal) * flexibleTarget -
        Math.floor((baseRolePlan[role] / flexibleBaseTotal) * flexibleTarget),
    }))
...
const expectedAccountTotal = Object.values(rolePlan).reduce(
  (sum, count) => sum + count,
  0,
);
...
const cleanupRunArg =
  process.argv
    .find((argument) => argument.startsWith("--cleanup-run="))
    ?.split("=")[1] || process.env.BETA_CLEANUP_RUN_ID;
...
function makePeople() {
  ...
  for (const [role, count] of Object.entries(rolePlan)) {
    for (let index = 1; index <= count; index += 1) {
      ...
      const lastName =
        lastNames[(people.length + index * 3) % lastNames.length];
...
const roleIds = await step("ensure roles", async () => {
  const rows = rolesToCreate.map(([name, display_name, description]) => ({
    name,
    display_name,
    description,
    deleted_at: null,
...
const { academicYear, terms } = await step(
  "create beta academic calendar",
  async () => {
...
const { departments, subjects, classrooms } = await step(
  ...
  async () => {
    const departmentRows = [
      ...
      code,
      name,
      description: `${name} beta department`,
    }));
    const existingDepartments = await dbSelect(
      "departments",
      "id,code,name,description",
    );
    const existingDepartmentCodes = new Set(
      existingDepartments.map((department) => department.code),
    );
    const insertedDepartments = await dbInsert(
      "departments",
      departmentRows.filter(
        (department) => !existingDepartmentCodes.has(department.code),
      ),
    );
    const createdDepartments = [
      ...existingDepartments.filter((department) =>
        departmentRows.some((row) => row.code === department.code),
      ),
      ...insertedDepartments,
    ];
    const academicDepartment = createdDepartments.find(
      (department) => department.code === "ACA",
    );
    const subjectNames = [
      ...new Set(classPlan.flatMap((item) => item.subjects)),
    ];
...
const people = await step(
  `create ${expectedAccountTotal} auth users and profiles`,
  async () => {
    ...
    for (let index = 0; index < planned.length; index += 1) {
      ...
      if (error)
        throw new Error(
          `auth create failed for ${person.email}: ${error.message}`,
        );
...
const staffProfiles = await step(
  "create staff records and class assignments",
  async () => {
    const staffPeople = people.filter(
      (person) => !["student", "parent"].includes(person.role),
    );
    const staffRows = staffPeople.map((person, index) => ({
      ...
      qualification_summary:
        "Synthetic beta staff account for full-platform readiness testing.",
      metadata: betaMarker,
    }));
...
const courses = await step(
  "create courses and teacher assignments",
  async () => {
    const subjectByName = Object.fromEntries(
      subjects.map((subject) => [subject.name, subject]),
    );
    const classroomByBaseName = Object.fromEntries(
      classrooms.map((classroom) => [
        classroom.name.replace(` Beta ${runId}`, ""),
        classroom,
      ]),
    );
    const teachers = peopleByRole.teacher ?? [];
    ...
    for (const [classIndex, classItem] of classPlan.entries()) {
      ...
      for (const [subjectIndex, subjectName] of classItem.subjects.entries()) {
        courseRows.push({
          ...
          classroom_id: classroom.id,
          teacher_id:
            teachers[(classIndex + subjectIndex) % teachers.length].id,
          academic_year_id: academicYear.id,
          term: "Term 1",
          deleted_at: null,
        });
    ...
    const createdCourses = await dbUpsert("courses", courseRows, {
      onConflict: "subject_id,classroom_id,academic_year_id,term",
    });
    await dbUpsert(
      "teacher_assignments",
...
const { students, parentLinks } = await step(
  `enroll ${rolePlan.student} students and link ${rolePlan.parent} parents`,
  async () => {
    ...
    const studentsByPerson = await dbInsert(
      ...
      studentPeople.map((person, index) => {
        ...
        return {
          profile_id: person.id,
          student_number: `CIS/ST/${String(index + 1).padStart(6, "0")}`,
          classroom_id: classroom.id,
    ...
    const links = await dbUpsert(
      ...
      studentsByPerson.map((student, index) => ({
        ...
        relationship:
          index % 3 === 0 ? "mother" : index % 3 === 1 ? "father" : "guardian",
        deleted_at: null,
      })),
    ...
    await dbInsert(
      ...
      classrooms.map((classroom) => {
        const roster = studentsByPerson
          ...
          .map((student) => {
            const profile = studentPeople.find(
              (person) => person.id === student.profile_id,
            );
            return {
              student_id: student.id,
...
const applications = await step(
  "simulate admissions and recruitment",
  async () => {
    const parentPeople = peopleByRole.parent ?? [];
        ...
        status: accepted
          ...
          : rejected
            ...
            : index % 2 === 0
              ...
              : "submitted",
        notes: accepted
          ? "Accepted during full beta admissions workflow."
          : "Pending or rejected beta admission workflow.",
        accepted_student_id: accepted ? linkedStudent.id : null,
        parent_profile_id: accepted ? parentProfile.id : null,
        generated_student_number: accepted
          ? linkedStudent.student_number
          : null,
        onboarding_notes: accepted
          ? `Parent account ready. Temporary password pattern validated for beta run ${runId}.`
          : null,
    ...
    const jobApps = await dbInsert(
      ...
      Array.from({ length: 18 }, (_, index) => ({
        ...
        cover_letter:
          "I am applying through the public recruitment flow for a beta readiness test.",
        status:
          index < 5
            ? "hired"
            : index < 8
              ...
              : index < 12
                ...
                : "submitted",
        assigned_to:
          (peopleByRole.hr_staff ?? [])[index % peopleByRole.hr_staff.length]
            ?.id ?? null,
...
const finance = await step(
  "simulate class billing, invoices, and payments",
  async () => {
    ...
    const planByClass = Object.fromEntries(
      dailyFeePlans.map((plan) => [plan.classroom_id, plan]),
    );
    const dailyFeeRows = [];
    for (let day = 0; day < 5; day += 1) {
      students.forEach((student, index) => {
        ...
        dailyFeeRows.push({
          ...
          recorded_by:
            peopleByRole.finance_officer[
              index % peopleByRole.finance_officer.length
            ].id,
          notes:
            paymentStatus === "waived"
              ? "Beta bursary waiver for daily fee workflow."
              : "Beta daily fee collected at finance desk.",
          metadata: {
            ...betaMarker,
            scan_source:
              (index + day) % 4 === 0 ? "student_qr_card" : "manual_student_id",
    ...
    const batches = await dbInsert(
      ...
      classrooms.map((classroom, index) => ({
        ...
        description:
          "Exceptional class-level billing retained for uniforms, resources, trips, and other non-daily charges.",
    ...
    const batchByClass = Object.fromEntries(
      batches.map((batch) => [batch.classroom_id, batch]),
    );
    const invoices = await dbInsert(
      ...
      students.map((student, index) => {
        ...
        const status =
          index % 5 === 0 ? "paid" : index % 7 === 0 ? "overdue" : "open";
        return {
          student_id: student.id,
...
const attendance = await step(
  "submit teacher attendance registers",
  async () => {
    const teachers = peopleByRole.teacher ?? [];
    const registers = [];
    const records = [];
    for (let day = 0; day < 5; day += 1) {
      for (const [classIndex, classroom] of classrooms.entries()) {
        ...
        const teacherId =
          course?.teacher_id ?? teachers[classIndex % teachers.length].id;
    ...
    for (const register of createdRegisters) {
      const classStudents = students.filter(
        (student) => student.classroom_id === register.classroom_id,
      );
      const course = courses.find(
        (item) => item.classroom_id === register.classroom_id,
      );
      classStudents.forEach((student, index) => {
        ...
        records.push({
          ...
          notes:
            status === "present" ? null : `Beta ${status} follow-up required.`,
...
const gradebook = await step(
  "publish class-specific 30/70 grading",
  async () => {
    const scaleRows = await dbSelect("grading_scales");
    const scaleByCode = Object.fromEntries(
      scaleRows.map((scale) => [scale.code, scale]),
    );
    const gradeItems = await dbInsert(
      ...
      courses.map((course) => {
        ...
        const classroom = classrooms.find(
          (item) => item.id === course.classroom_id,
        );
        return {
          course_id: course.id,
    ...
    const itemByCourse = Object.fromEntries(
      gradeItems.map((item) => [item.course_id, item]),
    );
    const gradeRows = [];
    for (const course of courses) {
      const subject = subjects.find((item) => item.id === course.subject_id);
      const classStudents = students.filter(
        (student) => student.classroom_id === course.classroom_id,
      );
      for (const [index, student] of classStudents.entries()) {
        const assignment = Math.min(
          10,
          5 + ((index + subject.name.length) % 6),
        );
        const quiz = Math.min(10, 4 + ((index * 2 + subject.name.length) % 7));
        const midterm = Math.min(
          10,
          4 + ((index * 3 + subject.name.length) % 7),
        );
        const classAssessment = assignment + quiz + midterm;
        const exam = Math.min(
          70,
          32 + ((index * 7 + subject.name.length * 3) % 39),
        );
            ...
            strengths:
              total >= 70
                ? [
                    ...
                    "Consistent class assessment performance",
                  ]
            ...
            concerns:
              total < 50
                ? [
                    ...
                    "Weak end-of-term exam score",
                  ]
                : [],
            recommendation:
              total < 50
                ? "Schedule teacher-parent intervention and targeted practice."
                : "Continue enrichment and revision routine.",
          },
        });
      }
    }
    ...
    await dbInsert(
      ...
      gradeItems.slice(0, 20).map((item, index) => {
        const course = courses.find(
          (courseRow) => courseRow.id === item.course_id,
        );
        const subject = subjects.find(
          (subjectRow) => subjectRow.id === course.subject_id,
        );
        return {
          ...
          status: "processed",
          rows_total: students.filter(
            (student) => student.classroom_id === course.classroom_id,
          ).length,
          rows_success: students.filter(
            (student) => student.classroom_id === course.classroom_id,
          ).length,
          rows_failed: 0,
          classroom_id: course.classroom_id,
...
const reportsAndAnalytics = await step(
  "generate reports, student 360 notes, and academic analytics",
  async () => {
    const gradesByStudent = Map.groupBy(
      gradebook.grades,
      ...
      students.map((student, index) => {
          ...
          studentGrades.length > 0
            ? Math.round(
                (studentGrades.reduce(
                  (sum, grade) =>
                    sum + Number(grade.total_score ?? grade.score),
                  0,
                ) /
        ...
        const attendanceRate =
          studentAttendance.length > 0
            ? Math.round((present / studentAttendance.length) * 1000) / 10
            : 0;
        return {
          student_id: student.id,
          academic_year_id: academicYear.id,
          ...
          report_url: null,
          generated_by:
            peopleByRole.teacher[index % peopleByRole.teacher.length].id,
          status: "published",
          published_at: new Date().toISOString(),
          analysis: {
            ...betaMarker,
            ...
            strengths:
              average >= 70
                ? ["Academic consistency", "Strong engagement"]
                : ["Shows willingness to improve"],
            weaknesses:
              average < 50 ? ["Needs more practice in core subjects"] : [],
            recommendations:
              average < 50
                ? ["Weekly remedial plan", "Parent-teacher conference"]
                : ["Maintain revision plan", "Stretch enrichment tasks"],
          },
          attendance_summary: {
            rate: attendanceRate,
            present,
          ...
          punctuality:
            attendanceRate >= 90
              ? "Very punctual"
              : "Needs punctuality support",
          next_steps:
            average < 50
              ? "Academic support task opened for teacher follow-up."
    ...
    await dbInsert(
      ...
      students.map((student) => {
        ...
        const average =
          studentGrades.length > 0
            ? studentGrades.reduce(
                (sum, grade) => sum + Number(grade.total_score ?? grade.score),
                0,
              ) / studentGrades.length
            : 0;
        return {
          ...
          recommendations:
            average < 50
              ? ["Remedial intervention", "Guardian engagement"]
              : ["Maintain steady monitoring"],
        };
      }),
    );
    await dbInsert(
...
const operations = await step(
  "exercise HR, library, IT, events, communication, and automation",
  async () => {
    ...
    await dbInsert(
      ...
      staffProfiles.slice(0, 12).map((staff, index) => ({
        ...
        reviewed_by:
          index % 4 === 0 || index % 5 === 0
            ? peopleByRole.hr_staff[0].id
            : null,
        reviewed_at:
          index % 4 === 0 || index % 5 === 0 ? new Date().toISOString() : null,
      })),
    );
      ...
      Array.from({ length: 12 }, (_, index) => ({
        ...
        title:
          [
            ...
            "Robotics for Young Learners",
            "Music Theory Foundations",
            "Ghanaian Language Reader",
          ][index % 4] + ` ${index + 1}`,
    ...
    await dbInsert(
      ...
      copies.slice(0, 30).map((copy, index) => ({
        ...
        issued_by:
          peopleByRole.librarian[index % peopleByRole.librarian.length].id,
    ...
    const devices = await dbInsert(
      ...
      Array.from({ length: 30 }, (_, index) => ({
        ...
        device_type: ["laptop", "tablet", "desktop", "music_equipment"][
          index % 4
        ],
    ...
    const tickets = await dbInsert(
      ...
      Array.from({ length: 30 }, (_, index) => ({
        ...
        assigned_to:
          peopleByRole.it_support[index % peopleByRole.it_support.length].id,
    ...
    await dbInsert(
      ...
      [
        ...
        "Music and Creative Arts Open Day",
        "Parent finance briefing",
      ].map((title, index) => ({
...
await step(
  "create notifications, messages, tasks, and lifecycle records",
  async () => {
    const invoiceByStudent = Map.groupBy(
      finance.invoices,
      (invoice) => invoice.student_id,
    );
    const parentByStudent = Object.fromEntries(
      parentLinks.map((link) => [link.student_id, link.parent_profile_id]),
    );
    await dbInsert(
      "notifications",
      people.map((person) => ({
    ...
    await dbInsert(
      ...
      conversations.flatMap((conversation, index) => {
        ...
        return [
          ...
          {
            conversation_id: conversation.id,
            profile_id: parentByStudent[student.id],
          },
    ...
    await dbInsert("workflow_tasks", [
      ...applications.admissions.slice(0, 10).map((application, index) => ({
        ...
        assigned_to:
          peopleByRole.school_admin[index % peopleByRole.school_admin.length]
            .id,
        ...
        classroom_id:
          students.find(
            (student) => student.id === application.accepted_student_id,
          )?.classroom_id ?? null,
      ...
      ...gradebook.grades
        ...
        .map((grade, index) => ({
          ...
          description:
            "Low score detected; teacher should prepare remediation.",
          ...
          classroom_id:
            students.find((student) => student.id === grade.student_id)
              ?.classroom_id ?? null,
    ...
    await dbInsert("account_lifecycle_records", [
      ...people.map((person) => ({
        profile_id: person.id,
        action: "created",
...
await step("audit beta platform readiness", async () => {
  const expectedCourseCount = classPlan.reduce(
    (sum, classroom) => sum + classroom.subjects.length,
    0,
  );
  ...
  students.forEach((student) => {
    studentCountByClassroom.set(
      ...
      (studentCountByClassroom.get(student.classroom_id) ?? 0) + 1,
    );
  });
  const expectedGradeCount = courses.reduce(
    (sum, course) =>
      sum + (studentCountByClassroom.get(course.classroom_id) ?? 0),
    0,
  );
  const expectedDailyFeePaymentMinimum = Math.floor(students.length * 5 * 0.9);
  const counts = {
    profiles: await countRows("profiles", (query) =>
      query.contains("metadata", { beta_run_id: runId }),
    ),
    students: await countRows("students", (query) =>
      query.contains("metadata", { beta_run_id: runId }),
    ),
    staffProfiles: await countRows("staff_profiles", (query) =>
      query.contains("metadata", { beta_run_id: runId }),
    ),
    admissions: await countRows("admission_applications", (query) =>
      query.contains("metadata", { beta_run_id: runId }),
    ),
    jobApplications: await countRows("job_applications", (query) =>
      query.contains("metadata", { beta_run_id: runId }),
    ),
    invoices: await countRows("invoices", (query) =>
      query.contains("metadata", { beta_run_id: runId }),
    ),
    ...
    activeStudentIdCards: await countRows("student_id_cards", (query) =>
      query
        .in(
          "student_id",
    ...
    attendanceRegisters: await countRows("attendance_registers", (query) =>
      query.contains("metadata", { beta_run_id: runId }),
    ),
    attendanceRecords: attendance.records.length,
    gradeItems: gradebook.gradeItems.length,
    grades: gradebook.grades.length,
    reports: await countRows("reports", (query) =>
      query.contains("metadata", { beta_run_id: runId }),
    ),
    notifications: await countRows("notifications", (query) =>
      query.contains("metadata", { beta_run_id: runId }),
    ),
    workflowTasks: await countRows("workflow_tasks", (query) =>
      query.contains("metadata", { beta_run_id: runId }),
    ),
    supportTickets: await countRows("support_tickets", (query) =>
      query.contains("metadata", { beta_run_id: runId }),
    ),
    devices: await countRows("devices", (query) =>
      query.contains("metadata", { beta_run_id: runId }),
    ),
    campaignRecipients: await countRows("communication_recipients", (query) =>
      query.contains("metadata", { beta_run_id: runId }),
    ),
  };
  report.accounts = Object.fromEntries(
    Object.entries(rolePlan).map(([role, expected]) => [
      role,
      { expected, actual: peopleByRole[role]?.length ?? 0 },
    ]),
  );
  report.counts = counts;
  const expectedStaffProfiles = people.filter(
    (person) => !["student", "parent"].includes(person.role),
  ).length;
  assertCheck(
    `created exactly ${expectedAccountTotal} portal profiles`,
    counts.profiles === expectedAccountTotal,
    { expected: expectedAccountTotal, actual: counts.profiles },
  );
  for (const [role, expected] of Object.entries(rolePlan)) {
    assertCheck(
      `role count: ${role}`,
      (peopleByRole[role]?.length ?? 0) === expected,
      { expected, actual: peopleByRole[role]?.length ?? 0 },
    );
  }
  assertCheck(
    `created ${rolePlan.student} students`,
    counts.students === rolePlan.student,
    { expected: rolePlan.student, actual: counts.students },
  );
  assertCheck(
    "linked every student to a guardian",
    parentLinks.length === students.length,
    { expected: students.length, actual: parentLinks.length },
  );
  assertCheck(
    "created staff profiles for every operational account",
    counts.staffProfiles === expectedStaffProfiles,
    { expected: expectedStaffProfiles, actual: counts.staffProfiles },
  );
  assertCheck(
    "class curriculum has courses for all required subjects including JHS",
    courses.length === expectedCourseCount,
    { expected: expectedCourseCount, actual: courses.length },
  );
  assertCheck(
    "JHS 1-3 curriculum includes the ten required junior high subjects",
    courses.filter((course) =>
      classrooms
        .find((classroom) => classroom.id === course.classroom_id)
        ?.grade_level?.startsWith("Junior High"),
    ).length === 30,
    {
      expected: 30,
      actual: courses.filter((course) =>
        classrooms
          .find((classroom) => classroom.id === course.classroom_id)
          ?.grade_level?.startsWith("Junior High"),
      ).length,
  ...
  assertCheck(
    "admissions workflow generated review records",
    counts.admissions === 24,
    { expected: 24, actual: counts.admissions },
  );
  assertCheck(
    "recruitment workflow generated applications",
    counts.jobApplications === 18,
    { expected: 18, actual: counts.jobApplications },
  );
  assertCheck(
    "finance generated daily fee plans for every class",
    counts.dailyFeePlans === classrooms.length,
    { expected: classrooms.length, actual: counts.dailyFeePlans },
  );
  assertCheck(
    "finance recorded daily QR/ID fee payments across the test week",
    counts.dailyFeePayments >= expectedDailyFeePaymentMinimum,
    {
      expectedAtLeast: expectedDailyFeePaymentMinimum,
      actual: counts.dailyFeePayments,
    },
  );
  assertCheck(
    "every active student has a QR ID card",
    counts.activeStudentIdCards === students.length,
    { expected: students.length, actual: counts.activeStudentIdCards },
  );
  assertCheck(
    "finance generated one special invoice per student",
    counts.invoices === students.length,
    { expected: students.length, actual: counts.invoices },
  );
  assertCheck("payments were recorded for paid invoices", counts.payments > 0, {
    actual: counts.payments,
  });
  assertCheck(
    "attendance registers cover each class for five days",
    counts.attendanceRegisters === classrooms.length * 5,
    { expected: classrooms.length * 5, actual: counts.attendanceRegisters },
  );
  assertCheck(
    "attendance records cover every student for five days",
    counts.attendanceRecords === students.length * 5,
    { expected: students.length * 5, actual: counts.attendanceRecords },
  );
  assertCheck(
    "professional 30/70 gradebook covers every class subject",
    counts.grades === expectedGradeCount,
    { expected: expectedGradeCount, actual: counts.grades },
  );
  assertCheck(
    "published one report per student",
    counts.reports === students.length,
    { expected: students.length, actual: counts.reports },
  );
  assertCheck(
    "notifications include all roles plus parent invoice alerts",
    counts.notifications >= expectedAccountTotal,
    { expectedAtLeast: expectedAccountTotal, actual: counts.notifications },
  );
  assertCheck(
    "workflow automation tasks were created",
    counts.workflowTasks >= 30,
    { expectedAtLeast: 30, actual: counts.workflowTasks },
  );
  assertCheck(
    "IT help desk tickets were exercised",
    counts.supportTickets === 30,
    { expected: 30, actual: counts.supportTickets },
  );
  assertCheck("device inventory was exercised", counts.devices === 30, {
    expected: 30,
    actual: counts.devices,
  });
  assertCheck(
    "communication campaign reached all profiles",
    counts.campaignRecipients === expectedAccountTotal,
    { expected: expectedAccountTotal, actual: counts.campaignRecipients },
  );
  const sampleParent = peopleByRole.parent[0];
  const sampleStudentLink = parentLinks.find(
    (link) => link.parent_profile_id === sampleParent.id,
  );
  const sampleInvoices = finance.invoices.filter(
    (invoice) => invoice.student_id === sampleStudentLink?.student_id,
  );
  const sampleDailyFees = finance.dailyFeePayments.filter(
    (payment) => payment.student_id === sampleStudentLink?.student_id,
  );
  const sampleReports = reportsAndAnalytics.reports.filter(
    (item) => item.student_id === sampleStudentLink?.student_id,
  );
  assertCheck(
    "sample parent sees linked ward daily fees, finance, and report data",
    Boolean(
      sampleStudentLink &&
      sampleDailyFees.length &&
  ...
  const teacherCourseCounts = Map.groupBy(
    courses,
    (course) => course.teacher_id,
  );
  assertCheck(
    "teachers received assigned courses",
    teacherCourseCounts.size >= 10,
    { assignedTeachers: teacherCourseCounts.size },
  );
  return counts;
});
...
process.stdout.write(
  `[beta:${runId}] Checks passed: ${report.checks.length - failedChecks.length}/${report.checks.length}\n`,
);
process.stdout.write(
  `[beta:${runId}] Profiles: ${report.counts.profiles}, students: ${report.counts.students}, grades: ${report.counts.grades}, attendance: ${report.counts.attendanceRecords}, daily fees: ${report.counts.dailyFeePayments}\n`,
);
